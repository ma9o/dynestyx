...
## Transition-only consumers

`discretize_state_evolution` is the same numerical operation used by
`discretize_dynamics`. Consumers with their own initial and observation models
can pass a resolved stochastic or deterministic continuous state evolution
without constructing an unrelated observation model. Local affine parameters
can likewise be requested from a stochastic state evolution directly.
