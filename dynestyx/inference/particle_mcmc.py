...
from cuthbert.mcmc.diagnostics import build_mpgibbs_diagnostic_flags
from cuthbert.mcmc.marginal_particle_gibbs import MarginalParticleGibbsKernel
from cuthbert.mcmc.state import TrajectoryMCMCState, _clip_scale

from dynestyx.inference import particle_profiling as _profiling
from dynestyx.inference.particle_chain import (
    _adapt_scale,
...
def _initialize_chain_state(
    ...
    *,
    ...
    predictive_latent = target.initial_path(context)
    latent_trajectory = (
        predictive_latent
    ...
    latent_delta_value = jnp.asarray(
        initial_latent_delta, dtype=latent_trajectory.dtype
    )
    ...
    da_init, _da_update, _ = dual_averaging_adaptation(
        target=float(param_target_accept)
    )
...
def run_marginal_particle_gibbs(
    ...
    *,
    ...
    if profile_runtime_trace and profile_trace_start_step >= total_steps:
        raise ValueError(
            "profile_trace_start_step must be less than the total step count."
        )
    if total_steps <= 0:
    ...
    if init_positions is None:
        ...
        chain_init_positions = (
            target.initial_position[None, :] + init_scale * init_noise
        )
    else:
        chain_init_positions = jnp.asarray(
            init_positions, dtype=target.initial_position.dtype
        )
    ...
    if initial_latent_trajectories is not None:
        chain_initial_latents = jnp.asarray(
            initial_latent_trajectories, dtype=observations.dtype
        )
        ...
        [
            _initialize_chain_state(
                ...
                initial_latent_trajectory=(
                    None
                    if chain_initial_latents is None
                    else chain_initial_latents[chain_idx]
                ),
            )
    ...
    public_latent_fn = lambda context, path, observations, key: path
    ...
    score_observations = jax.jit(
        jax.vmap(
            lambda context, path: target.observation_log_probs(
                context, path, observations
            )
        )
    )
    ...
    try:
        for step_idx in range(total_steps):
            ...
            if (
                ...
                param_accept_now = jax.device_get(
                    jnp.mean(step_info["parameter_accepted"])
                )
                latent_accept_now = jax.device_get(
                    jnp.mean(step_info["latent_accepted"])
                )
            ...
            latent_accept_history.append(
                jnp.mean(step_info["latent_accepted"], axis=-1)
            )
            ...
            if diagnostic_flags.particle_identity:
                selected_particle_per_t_history.append(
                    step_info["selected_particle_per_t"]
                )
                reference_path_hit_rate_history.append(
                    step_info["reference_path_hit_rate"]
                )
            ...
            if step_idx < num_warmup:
                if kernel.adapt_amala_delta:
                    ...
                    latent_acceptance_window = latent_acceptance_window.at[
                        :, window_slot, :
                    ].set(
                        step_info["latent_accepted"].astype(
                            latent_acceptance_window.dtype
                        )
                    )
                    ...
                    learning_rate = jnp.maximum(
                        ...
                        * jnp.asarray(
                            kernel.amala_adaptation_rho, dtype=states.latent_delta.dtype
                        ),
                        jnp.asarray(
    ...
    warmup_complete_log_posterior_history = all_complete_log_posterior_history[
        :, :num_warmup
    ]
